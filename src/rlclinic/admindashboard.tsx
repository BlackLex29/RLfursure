 ...
 interface UnavailableSlot {
  ...
  endTime?: string;
  reason?: string;
  leaveDays?: number;      // ← ADD THIS
  endDate?: string;         // ← ADD THIS
  isMultipleDays?: boolean; // ← ADD THIS
}
  ...
  setIsSendingOTP(false);
    ...
    const fetchAppointments = useCallback(() => {
      ...
      const unsubscribe = onSnapshot(appointmentsRef, (snapshot) => {
        ...
        // FIXED: Better today's date filtering with proper timezone handling
        const today = new Date();
        ...
        console.log("📊 Today's appointment details:", todayAppts.map(apt => ({
          id: apt.id,
          date: apt.date,
...
{viewMode === "unavailable" && (
  <AppointmentsSection>
    ...
    {filteredUnavailableSlots.length === 0 ? (
      ...
      <AppointmentsGrid>
        {(() => {
          // Group multiple day leaves together
          const groupedSlots: { [key: string]: UnavailableSlot[] } = {};
          const processedIds = new Set<string>();

          filteredUnavailableSlots.forEach((slot) => {
            if (processedIds.has(slot.id)) return;

            if (slot.isMultipleDays && slot.leaveDays && slot.leaveDays > 1) {
              // This is part of a multiple day leave
              const groupKey = `${slot.date}-${slot.leaveDays}-${slot.reason}`;
              
              if (!groupedSlots[groupKey]) {
                groupedSlots[groupKey] = [];
              }
              
              // Find all slots that belong to this leave period
              const leaveDays = slot.leaveDays || 1; // Fallback to 1 day
              const relatedSlots = filteredUnavailableSlots.filter(s => 
                s.isMultipleDays && 
                s.leaveDays === slot.leaveDays &&
                s.reason === slot.reason &&
                Math.abs(new Date(s.date).getTime() - new Date(slot.date).getTime()) < leaveDays * 24 * 60 * 60 * 1000
              );
              
              relatedSlots.forEach(s => {
                if (!processedIds.has(s.id)) {
                  groupedSlots[groupKey].push(s);
                  processedIds.add(s.id);
                }
              });
            } else {
              // Single day leave
              const groupKey = `single-${slot.id}`;
              groupedSlots[groupKey] = [slot];
              ...
              <UnavailableCard key={`group-${index}`} $delay={index * 0.1}>
                ...
                <UnavailableActions>
                  <ActionButton
                    $variant="primary"
                    onClick={() => {
                      const details = isMultipleDays
                        ? `Multiple Day Leave:\n\n` +
                          `Veterinarian: ${firstSlot.veterinarian}\n` +
                          `Period: ${dateDisplay}\n` +
                          `Duration: ${slots.length} days\n` +
                          `Time: ${firstSlot.isAllDay ? 'All Day' : `${firstSlot.startTime} - ${firstSlot.endTime}`}\n` +
                          `Reason: ${firstSlot.reason || 'Not specified'}`
                        : `Single Day Leave:\n\n` +
                          `Veterinarian: ${firstSlot.veterinarian}\n` +
                          `Date: ${dateDisplay}\n` +
                          `Time: ${firstSlot.isAllDay ? 'All Day' : `${firstSlot.startTime} - ${firstSlot.endTime}`}\n` +
